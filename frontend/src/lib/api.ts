...
export const getPrescriptions = (params?: Record<string, string>) => {
  ...
  return fetchApi<Prescription[]>(`/prescriptions${qs}`);
};

export const getPrescription = (id: string) =>
  fetchApi<PrescriptionDetail>(`/prescriptions/${id}`);

export const prescriptionAction = async (id: string, action: string, reason?: string) => {
  const res = await fetchApi<{ status: string }>(`/prescriptions/${id}/action`, {
    method: "POST",
...
export const analyzePrescription = (data: {
  ...
  fetchApi<PrescriptionDetail>("/prescriptions/analyze", {
    method: "POST",
...
export const getPatient = (id: string) => fetchApi<PatientDetail>(`/patients/${id}`);
export const getPatientMedications = (id: string) =>
  fetchApi<ActiveMedication[]>(`/patients/${id}/medications`);

// Providers
export const getProviders = () => fetchApi<Provider[]>("/providers");
export const getProvider = (id: string) => fetchApi<ProviderDetail>(`/providers/${id}`);

// Analytics
export const getOverview = () => fetchApi<OverviewMetrics>("/analytics/overview");
export const getSavings = () => fetchApi<SavingsData[]>("/analytics/savings");
export const getTrends = () => fetchApi<TrendData[]>("/analytics/trends");
export const getFraud = () => fetchApi<FraudMetrics>("/analytics/fraud");
...
export const getAuditTrail = (params?: Record<string, string>) => {
  const qs = params ? `?${new URLSearchParams(params)}` : "";
  return fetchApi<AuditTrailResponse>(`/audit${qs}`);
};
export const getAuditStats = () => fetchApi<AuditStats>("/audit/stats");

// Interaction Network
export const getInteractionNetwork = (patientId?: string) => {
  const qs = patientId ? `?patient_id=${patientId}` : "";
  return fetchApi<InteractionNetwork>(`/interactions/network${qs}`);
};
export const checkInteraction = (drugAId: string, drugBId: string) =>
  fetchApi<any>(`/interactions/check?drug_a_id=${drugAId}&drug_b_id=${drugBId}`);
