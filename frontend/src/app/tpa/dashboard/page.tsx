    ...
    <div className="space-y-12 max-w-[1180px]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <Panel title="Where the money leaks" sub="Annualized, largest first"
               action={<Link href="/tpa/pbm-audit" className="text-[12px] text-blue-600 dark:text-blue-400 inline-flex items-center gap-1">Audit <ArrowRight className="w-3 h-3" /></Link>}>
          <LeakBars rows={leakRows} />
        </Panel>
        <Panel title="Waiting on you" sub="Open items that need a decision">
          <div className="-mx-2">
            {[
              { href: "/tpa/asa-disputes", label: "Open ASA disputes", count: data.asa_disputes_open, sub: "X12 276/277 filed against the PBM" },
              { href: "/tpa/fraud-referrals", label: "Fraud referrals", count: data.fraud_referrals_open, sub: "LEIE and SAM.gov plus ML signals" },
              { href: "/tpa/conflict-audit", label: "Conflict of interest", count: cc.conf ? `${cc.conf.vertical_integration.affiliated_share_pct}%` : "·", sub: "Affiliated steering and broker comp" },
              { href: "/tpa/pend-queue", label: "Hard holds to resolve", count: data.hard_holds, sub: "Explicit resolution required" },
            ].map((it) => (
      ...
      <section data-tour="pend">
        <div className="flex items-baseline justify-between mb-6">
          ...
          <Link href="/tpa/pend-queue" className="text-[13px] text-blue-600 dark:text-blue-400 inline-flex items-center gap-1 hover:gap-1.5 transition-all">
