  ...
  return (
    <div>
      ...
      <DetailDrawer open={!!selected} onClose={() => setSelected(null)}
        ...
        actions={detail && (
          <>
            <Link href={`/prescriptions/${detail.rx_id}`} className="px-3 py-1.5 text-[13px] rounded border border-slate-300 hover:bg-slate-50">
              Open Full Claim
            </Link>
            <Link href={`/pba/callbacks`} className="px-3 py-1.5 text-[13px] rounded bg-blue-600 text-white">
              Open Callback Queue
            </Link>
          </>
        )}
