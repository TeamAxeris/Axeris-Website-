...
function TPAPendQueueInner() {
  ...
  const columns: Column<PendItem>[] = [
    { key: "rx", header: "Rx ID", width: "100px",
      render: (i) => <Link href={`/prescriptions/${i.rx_id}`} className="font-mono text-[12px] text-blue-600 hover:underline dark:text-blue-400">{i.rx_id}</Link> },
    ...
    <div>
      ...
      {breachParam && (
        <div className="mb-3 flex items-center justify-between bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded px-3 py-2 text-[12px]">
          ...
          <Link href="/tpa/pend-queue" className="text-amber-700 dark:text-amber-300 hover:underline font-semibold">Clear filter</Link>
        </div>
      )}
      ...
      <DataTable columns={columns} rows={visibleItems} rowKey={(i) => i.rx_id}
        ...
        onRowClick={(i) => window.location.href = `/prescriptions/${i.rx_id}`} />
    </div>
  );
}
